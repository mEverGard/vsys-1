...
int saveEmail(std::string username, std::vector<std::string> message, int subjectRow, std::string receiver, char *dir, int soc, int main, bool sent){
    ...
    std::ofstream MailFile(filePath);
        if (main == 0){
            ...
            int n = 2;
            MailFile << "RECEIVER: ";
            if (subjectRow == 3) MailFile << message[n] << std::endl;
...
void sendHandler(std::vector<std::string> message, char *dir, int soc, std::string username)
{
    ...
    //Check if multiple receivers.
        ...
        //Main email
            if ((mainRow = saveEmail(username, message, subjectRow, "Pool", dir, soc, 0, false)) == -1) out = status_code[1];
        //Save references
            for (int i = 1; i < count + 1; i++){
                if (saveEmail("Pool", message, subjectRow, message[i+1], dir, soc, mainRow, false) == -1) out = status_code[1];
            }
        //Sent emails
            if (saveEmail(username, message, subjectRow, username, dir, soc, mainRow, true) == -1) out = status_code[1];
